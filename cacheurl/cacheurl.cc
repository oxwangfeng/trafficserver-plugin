...
// the header we should intercept
#define TS_SLICE_INNER_RANGE_HEADER "T-Slice-Range"
...
struct pr_list {
  ...
  ~pr_list()
  {
    for (std::vector<regex_info *>::iterator info = this->pr.begin(); info != this->pr.end();) {
      TSfree((*info)->tokens);
...
static int
regex_substitute(TSHttpTxn txnp, char **buf, char *str, regex_info *info)
{
  int matchcount;
  ...
  /* use header if header flag is on */
  if (info->use_header) {
    TSMBuffer bufp;
    TSMLoc hdr_loc;
    TSMLoc filed_loc;
    ...
    filed_loc = TSMimeHdrFieldFind(bufp, hdr_loc, info->replacement, strlen(info->replacement));
    if (filed_loc != NULL) {
      const char *value = TSMimeHdrFieldValueStringGet(bufp, hdr_loc, filed_loc, -1, &len);
      if (value != NULL && len != 0) {
        *buf = (char *)TSmalloc(len + 1);
        memset(*buf, 0, len + 1);
        memcpy(*buf, value, len);

        TSHandleMLocRelease(bufp, hdr_loc, filed_loc);
...
static int
regex_compile(regex_info **buf, char *pattern, char *replacement, bool use_header)
{
...
load_config_file(const char *config_file)
{
  ...
  while (TSfgets(fh, buffer, sizeof(buffer) - 1)) {
    ...
    *spstart = 0;
    ...
    if (*spend == '-' && strncmp(spend, "--header=", 8) == 0) {
      spend += strlen("--header=");
      TSDebug(PLUGIN_NAME, "Adding pattern/header pair: '%s' -> '%s'", buffer, spend);
      retval = regex_compile(&info, buffer, spend, true);
      if (!retval) {
      ...
      retval = regex_compile(&info, buffer, spend, false);
      if (!retval) {
        TSError("[%s] Error precompiling regex/replacement. Skipping.", PLUGIN_NAME);
        TSfclose(fh);
        return NULL;
...
*/
  ...
  field_loc = TSMimeHdrFieldFind(bufp, hdr_loc, TS_SLICE_INNER_RANGE_HEADER, strlen(TS_SLICE_INNER_RANGE_HEADER));
  if (field_loc) {
    ok = 1;
...
rewrite_cacheurl(pr_list *prl, TSHttpTxn txnp)
{
  ...
  if (internal_check(txnp)) {
    // if the request is internal request, just return!(only slice plugin currently)
    return 0;
  }
  ...
  if (ok) {
    for (std::vector<regex_info *>::iterator info = prl->pr.begin(); info != prl->pr.end(); ++info) {
      retval = regex_substitute(txnp, &newurl, url, *info);
      if (retval) {
    ...
    if (newurl) {
      ...
      if (TSCacheUrlSet(txnp, newurl, strlen(newurl)) != TS_SUCCESS) {
        TSDebug(PLUGIN_NAME, "[%s] Unable to modify cache url from "
                             "%s to %s",
...
static int
manager_update(TSCont contp, TSEvent event, void *edata)
{
  pr_list *newpr = NULL, *oldpr = NULL;
  TSCont ncont = NULL;
  switch ((int)event) {
  case TS_EVENT_MGMT_UPDATE:
    newpr = load_config_file(filename);
    if (newpr == NULL) {
      TSError("[%s] cacheurl reload faile", PLUGIN_NAME);
      return 0;
    }

    oldpr = (pr_list *)ink_atomic_swap_ptr(&prl, newpr);
    ncont = TSContCreate(manager_update, TSMutexCreate());
    TSContDataSet(ncont, oldpr);
    TSContSchedule(ncont, RELEASE_TIME, TS_THREAD_POOL_DEFAULT);
    break;
  case TS_EVENT_TIMEOUT:

    oldpr = (pr_list *)TSContDataGet(contp);
    if (oldpr != NULL)
      delete oldpr;
    TSContDestroy(contp);
    break;
...
TSPluginInit(int argc, const char *argv[])
{
  ...
  if (argc > 1) {
    filename = TSstrdup(argv[1]);
  }

  prl = load_config_file(filename);
  if (prl) {
    TSCont new_contp = TSContCreate(manager_update, NULL);
    TSMgmtUpdateRegister(new_contp, PLUGIN_NAME);
