...
#define MP4_TYPE_TIME   0
#define MP4_TYPE_SIZE   1
...
public:
    Mp4Trak(): timescale(0), duration(0), time_to_sample_entries(0), sample_to_chunk_entries(0),
               ...
               chunks(0), start_sample(0), end_sample(0),
               start_chunk(0), end_chunk(0),
               chunk_samples(0),chunk_samples_size(0),
               end_chunk_samples(0), end_chunk_samples_size(0),
               start_offset(0), end_offset(0),
               tkhd_size(0), mdhd_size(0), hdlr_size(0),
...
public:
    ...
    mp4_stsc_entry  stsc_end_chunk_entry;
};

class Mp4Meta
{
public:
    Mp4Meta(): start(0), end(0) ,length(0), cl(0),
                  type(MP4_TYPE_TIME),
                  content_length(0), meta_atom_size(0),
                  meta_avail(0), wait_next(0), need_size(0), rs(0), rate(0), ls(0),
                  ftyp_size(0), moov_size(0), start_pos(0),
                  timescale(0), trak_num(0),
    ...
    int mp4_get_time_by_offset(Mp4Trak *trak);
    int mp4_get_keyframe_time(Mp4Trak *trak, uint32_t sample, double *time);
    int mp4_get_keyframe_offset(Mp4Trak *trak, uint32_t sample, double *offset);

    int mp4_update_stts_atom(Mp4Trak *trak);
    int mp4_crop_stts_atom(Mp4Trak *trak);
    int mp4_update_stss_atom(Mp4Trak *trak);
    int mp4_crop_stss_atom(Mp4Trak *trak);
    int mp4_update_ctts_atom(Mp4Trak *trak);
    int mp4_crop_ctts_atom(Mp4Trak *trak);
    int mp4_update_stsc_atom(Mp4Trak *trak);
    int mp4_crop_stsc_atom(Mp4Trak *trak);
    ...
    void mp4_print_sync_value(Mp4Trak *trak);
...
public:
    ...
    int64_t             end;                    // requested end time, measured in milliseconds.
    int64_t             length;                 // this->end -  this->start
